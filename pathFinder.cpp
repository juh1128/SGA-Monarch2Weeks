...
//		if (_currentNode->_tile->_index.x + j < 0 || _currentNode->_tile->_index.y + i < 0 ||
//			_currentNode->_tile->_index.x + j >= WORLD->getMapSize().x ||
//			_currentNode->_tile->_index.y + i >= WORLD->getMapSize().y)
//			return false;
//
//		terrainTile* tile = WORLD->getTerrainTile(_currentNode->_tile->_index.x + j, _currentNode->_tile->_index.y + i);
...
//
//		//해당 타일이 이동불가 타일일 경우.
//		if (!tile->_isPossibleMove)
//		{
//			return false;
//		}
//		
//		//해당 타일이 벽은 아닌데~
//		//만약 노드의 방향이 대각선일 경우 해당 방향으로 이동하는쪽에 장애물 타일이 있는지 확인.
//		//만약 장애물 타일이 있다면 이 타일은 이동 불가하므로 무시한다.
//		int xDistance = tile->_index.x - _currentNode->_tile->_index.x;
//		int yDistance = tile->_index.y - _currentNode->_tile->_index.y;
//		//좌상단쪽으로 대각선
//		if (xDistance == -1 && yDistance == -1)
//		{
//			if (!WORLD->isPossibleMove(tile->_index.x, tile->_index.y + 1))
//				return false;
//			if (!WORLD->isPossibleMove(tile->_index.x + 1, tile->_index.y))
//				return false;
//		}
//		//우상단쪽으로 대각선
//		if (xDistance == 1 && yDistance == -1)
...
//
//	
//	int xIndex = _currentNode->_tile->_index.x;
//	int yIndex = _currentNode->_tile->_index.y;
//
//	add(- 1, 0); //좌
//	add(1, 0); //우
//	add(0, - 1); //상
...
//	if (!dest->_isPossibleMove) return path;
...
pathFinder::tagAstarNode* pathFinder::getChildNode(tagAstarNode* parentNode, terrainTile * childTile)
{
	...
	//if (abs(childTile->_index.x - parentNode->_tile->_index.x) +
	//	abs(childTile->_index.y - parentNode->_tile->_index.y) == 1)
	//{
	...
	//node->_costToGoal = (abs(_destTile->_index.x - childTile->_index.x) +
	//	abs(_destTile->_index.y - childTile->_index.y))*10;
